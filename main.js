...
const moment = require("moment-timezone");
    ...
    if (time2 < "24:59:00") {
      var ucapanWaktu = "Selamat malam Jangan Lupa Tidur";
    }
    if (time2 < "19:00:00") {
      var ucapanWaktu = "Selamat senja Jangan Lupa Sholat";
    }
    if (time2 < "18:00:00") {
      var ucapanWaktu = "Selamat sore Jangan Lupa Ngoding";
    }
    if (time2 < "15:00:00") {
      var ucapanWaktu = "Selamat siang Jangan Lupa Makan";
    }
    if (time2 < "11:00:00") {
      var ucapanWaktu = "Selamat pagi Jangan Lupa Olahraga";
    }
    if (time2 < "05:00:00") {
      var ucapanWaktu = "Selamat malam Jangan Lupa Tidur";
    }
...
console.log(color('WELCOME MASTAH FADILA!!','orange'))
console.log(color('SC BOT BUATAN MASTAH FADILA😊😋😉','pink'))
console.log(color('HALO FADILA!! SELAMAT MENJALANKAN BOT😊','yellow'))
const starts = async (client = new WAConnection()) => {
  client.logger.level = "warn";
  client.version = [2, 2123, 8];
  client.browserDescription = ["FADILA", "Mastah", "3.0"];
  ...
  client.on("group-participants-update", async (anu) => {
    try {
      ...
      if (anu.action == "remove" && !mem.includes(client.user.jid)) {
        ...
        time_wel = moment.tz("Asia/Jakarta").format("HH:mm");
        memeg = mdata.participants.length;
        out = `SELAMAT TINGGAL ${anu_user}\nJasa Mu Akan Saya Buang Ke Sampah👋🗿`;
        buff = await getBuffer(
        ...
        buttons = [
          { buttonId: `y`, buttonText: { displayText: "SELAMAT TINGGAL👋" }, type: 1 },
        ];
        imageMsg = (
          await client.prepareMessageMedia(buff, "imageMessage", {
            thumbnail: buff,
      ...
      if (anu.action == "demote") {
        ...
        teks = `Kasihan ${anu_user} Telah didemote`;
        client.sendMessage(mdata.id, buffer, MessageType.image, {
          caption: teks,
        });
      }
